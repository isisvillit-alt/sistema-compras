...
# ==================================================
# SINCRONIZAR COMPRAS GUARDADAS SIN INTERNET
# ==================================================

@app.route(
    "/api/sincronizar_compra",
    methods=["POST"],
)
def sincronizar_compra():
    if "user_id" not in session:
        return jsonify(
            {
                "ok": False,
    ...
    # Comprueba si esta compra ya se sincronizó.
    try:
        with get_db() as db:
            with db.cursor() as cursor:
                cursor.execute(
        ...
        app.logger.exception(
            "Error verificando compra pendiente"
        )
    ...
    try:
        ...
        app.logger.exception(
            "Error subiendo evidencia pendiente"
        )
